...
async function getTargetRepoToken(
	...
	if (context.type === "gitlab-ci" && platform.platform === "gitlab") {
		...
		return {
			error: "In GitLab CI but no authentication available. Set GL_TOKEN, GITLAB_TOKEN, or use CI_JOB_TOKEN.",
		}
	}
	...
	async execute(args, ctx: ToolContext) {
		...
		try {
			switch (args.operation) {
				case "clone":
					return stringify(await cloneRepo(ctx.sessionID, args.owner, args.repo, platform, args.branch))
